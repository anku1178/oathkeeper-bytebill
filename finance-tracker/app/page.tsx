...
export default function Dashboard() {
  const [activeTimeframe, setActiveTimeframe] = useState<"monthly" | "weekly">("monthly")

    // Dynamic user profile, streak, points, badges, challenge
  ...
  const streakData = profileData.streak;
  const pointsData = {
    total: profileData.points,
    level: profileData.level,
    nextLevel: 3000, // You may want to fetch this dynamically too
    recentlyEarned: 75, // Or fetch dynamically
  };
  type BadgeType = {
  id: string | number;
  name: string;
...
const recentBadges: BadgeType[] = (profileData.badges || []).map((badge: any) => ({
  ...badge,
  icon: badge.icon === "trophy" ? <Trophy className="h-5 w-5" /> : badge.icon === "flame" ? <Flame className="h-5 w-5" /> : <Award className="h-5 w-5" />,
}));
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex justify-between items-center mb-4">
              {recentBadges.map((badge: BadgeType) => (
